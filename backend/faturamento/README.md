...
- **Health check:** [http://localhost:8081/health](http://localhost:8081/health) — `200` se o serviço e
...
de erro internamente desde a primeira versão; a diferenciação até o
handler foi propagada junto com a correção descrita abaixo.
...
Essa compensação (padrão *saga* / transação compensatória) é o que
garante que o sistema nunca fica em um estado intermediário
inconsistente: ou a nota foi 100% processada (todos os itens baixados
e nota `FECHADA`), ou nenhum saldo permanece baixado e a nota continua
`ABERTA` para nova tentativa.
...
A compensação é "melhor esforço": se o próprio estorno falhar (ex: o
Estoque cai durante a compensação), a falha é registrada em log para
reconciliação manual, em vez de ser perdida silenciosamente. Uma
evolução futura possível é persistir esses eventos numa tabela de
compensações pendentes e reprocessá-los via job assíncrono — não
implementado nesta versão, mas documentado como próximo passo natural.
...
Este roteiro prova especificamente a compensação: cria uma nota com um
item válido e outro inexistente no Estoque, confirmando que o saldo do
item válido volta ao normal após a falha.
...
| 15  | Saldo insuficiente em um item                                    | `422`, item anterior compensado (ver acima)      |
